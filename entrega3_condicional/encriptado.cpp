/**
 ...
 int main () {
  ...
  char let, orden;
  char resultado, minuscula, mayuscula;
  int cod;
  cin >> orden >> cod >> let;

if ( 97 <= let <= 122 || 65 <= let <= 90)
// (let < 97 || let > 122 || let < 65 || let > 90)
...
{ 
     ...
     switch (orden)
     {
         ...
         //Funcionando
         if (let + cod >=97 && let + cod <=122) {
             char letminuscula = let + cod;
             resultado = letminuscula;
         }
        
        // Funciona.
         if (let + cod>122 /*&& let + cod < 97*/) {
             char letminuscula= ((let + cod)-26);
             resultado = letminuscula;
         }

        // Funciona.
         if (let+cod>=65 && let+cod<=90){
             char mayuscula= let + cod;
             resultado =  mayuscula;
         }

        //Funciona.
         if (let+cod>90 /*&& let + cod < 65*/){
             char mayuscula = (let + cod - 26);
             resultado = mayuscula;
         }
         break;
         ...
         //Funciona.  Desencriptar mayuscula -A
         if (let-cod < 65){
             mayuscula = (let - cod + 26);
             resultado = mayuscula;
         }
         break;
     }
